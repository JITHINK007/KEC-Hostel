	  ...
	  <ul class="navbar-nav mr-auto sidenav" id="navAccordion"  >
		...
		<li class="nav-item active">
		  ...
		  <ul class="nav-second-level collapse" id="collapseSubItems2" data-parent="#navAccordion">
			<li class="nav-item active" style="background-color:#2196f3;">
...
<!--===================================================================================================================================================-->
<div>
	<h1 style="margin:-3% 0% 0% 0%;">Sick Statistics:</h1>
</div>
<!---<?PHP
include "connect.php";
...
$query2="SELECT COUNT(`rollnumber`) AS `total` FROM `perpermissions_details` WHERE (SELECT DISTINCT(`rollnumber`) FROM `perpermissions_details` WHERE `leavingdatetime`!=''AND `arrival_time`='-')";
...
<center>
	<div class="main">
		...
		<?PHP
			include 'connect.php';
			$sql = "SELECT `leavingdatetime` FROM `perpermissions_details` WHERE `status`='ACCEPTED' AND  `prmissiontype`='Sick Permission'";
			...
			while($row = mysqli_fetch_array($query)){
				$year = substr($row['leavingdatetime'],0,4);
				$month = substr($row['leavingdatetime'],5,2);
				$day = substr($row['leavingdatetime'],8,2);
